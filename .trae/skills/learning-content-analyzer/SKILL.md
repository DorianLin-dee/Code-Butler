...
**转录时优先调用豆包 ASR 2.0 API，不要让用户手动操作。**

| 优先级 | 方式 | 触发条件 | 命令 |
|--------|------|---------|------|
| **1（默认）** | 🤖 豆包 ASR 2.0 API | 默认行为，无需任何参数 | `learning_pipeline.py "URL"` |
| **2（备选）** | 🎙️ 本地 Whisper | 用户明确要求离线/无网络 | `learning_pipeline.py "URL" --local` |
| **3（备选）** | 📋 手动豆包网页版 | API 不可用且本地 Whisper 不可用 | `learning_pipeline.py "URL" --manual` |

**AI 行为准则：**
- 用户给链接时，**默认用 API 转录**，不要询问用户选择哪种方式
- 只有在 API 失败时，才回退到本地 Whisper，再回退到手动模式
- API Key 已配置在 `api_key.txt`，无需向用户索取
- 不要主动建议用户用豆包网页版手动转录，除非 API 明确不可用
